package om.openjr.auth.token.creator;
...
import om.openjr.auth.model.UserAccount;
...
public class TokenCreator {
    ...
    public SignedJWT createSignedJWT(Authentication auth) {
        ...
        final UserAccount applicationUser = (UserAccount) auth.getPrincipal();

        final JWTClaimsSet jwtClaimSet = this.createJWTClaimSet(auth, applicationUser);

        final KeyPair rsaKeys = this.generateKeyPair();
    ...
    private JWTClaimsSet createJWTClaimSet(Authentication auth, UserAccount applicationUser) {
        ...
        return new JWTClaimsSet.Builder()
                ...
                .issuer("https://virtus.ufcg.edu.br")
                .issueTime(new Date())
    ...
    public String encryptToken(SignedJWT signedJWT) throws JOSEException {
        ...
        final DirectEncrypter directEncrypter = new DirectEncrypter(this.jwtConfiguration.getPrivateKey().getBytes());
